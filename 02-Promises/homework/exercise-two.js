...
let p2 = promisifiedReadFile('poem-two/stanza-02.txt').then(stanza2 => 
  ...
  Promesi.all([ p1, p2]).then (() => console.log('done'));
...
function problemB () {
 ...
 // );
  ...
  // promise version
 let promesiArray = filenames.map(file => promisifiedReadFile(file).then(stanza => blue(stanza)))
Promise.all(promesiArray).then(() => console.log('done'));
...
function problemE () {
  ...
  function promisifiedWriteFile (filename, str) {
    ...
    return new Promise((resolve, reject) =>{
      fs.writeFile(filenames, str, (err) =>{
        if (err) reject (err);
